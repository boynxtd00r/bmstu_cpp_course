   ...
   public:
    ...
    basic_string(const basic_string& other) : is_long_(other.is_long_)
    {
        if (is_long_) {
            ...
            set_long_ptr(ptr, other.data_.long_str.size, other.data_.long_str.capacity);
        } else {
    ...
    basic_string& operator=(const basic_string& other)
    {
        if (this != &other) {
            if (other.is_long_) {
                if (is_long_ && data_.long_str.capacity >= other.data_.long_str.size) {
                    for (size_t i = 0; i < other.data_.long_str.size; ++i) {
                                     ...
                                     const basic_string<T>& right)
    {
        ...
        if (total_size <= SSO_CAPACITY) {
            ...
            for (size_t i = 0; i < right_size; ++i) {
                result.data_.short_str.buffer[left_size + i] = right.get_ptr()[i];
            }
    ...
    friend S& operator>>(S& is, basic_string& obj)
    {
        ...
        while (!is.eof()) {
            ...
            if (new_total_size <= SSO_CAPACITY) {
                ...
                const T* old_ptr = temp.is_long_ ? temp.data_.long_str.ptr : temp.data_.short_str.buffer;
